...
export default function FaceRecognition({ savedFaces }: Props) {
      ...
      const labeledDescriptors = savedFaces.reduce(
        (acc: faceapi.LabeledFaceDescriptors[], face) => {
          ...
          if (existing) {
            ...
            acc.push(
              new faceapi.LabeledFaceDescriptors(face.label.name, [
                face.descriptor,
              ])
            );
          }
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full">
      ...
      {/* Only show the rest of the UI if we're not focusing on the webcam view */}
      {false && (
        <>
